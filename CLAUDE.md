...
- ⏳ **RESTE (IDEA, Mac + final)** : (a) `npm i @revenuecat/purchases-capacitor@^13` + `npx cap sync` + `npm run build` + git push ; (b) capture d'écran de review pour CHAQUE abo dans ASC (réutiliser `iap-review-screenshot.png`) ; (c) rattacher les 2 abos à la version + soumettre AVEC le build ; (d) archive Xcode + upload.
