...
/**
 * La page est montée par DEUX routes :
 *  · /bilan   → suivi + calculateurs (onglets Bilan / Calculateur)
 *  · /profile → informations personnelles uniquement (aucun onglet)
 * Toute la logique et les états restent partagés — seule la vue change.
 */
export type ProfileMode = 'bilan' | 'profil'
type CalcTab = 'tdee' | 'imc' | 'macros'

export function ProfileBilan({ mode }: { mode: ProfileMode }) {
  const router   = useRouter()
  const supabase = createClient()
  const { step: obStep, advance: obAdvance, skip: obSkip } = useOnboarding()

  // ── États principaux ────────────────────────────────────────────────────────
  const [tab, setTab]         = useState<Tab>(mode === 'profil' ? 'profil' : 'bilan')
  ...
  useEffect(() => {
    ...
    if (params.get('section') === 'rapport' && mode === 'bilan') {
      setTab('bilan')
      setTimeout(() => {
  ...
  return (
    <div className="page">
      ...
      {/* Onglets — uniquement sur /bilan ; /profile n'a plus qu'une seule vue */}
      {mode === 'bilan' && (
        <div className="flex bg-zinc-100 dark:bg-zinc-800 rounded-xl p-1 gap-1">
          {([
            { key: 'bilan',       label: 'Bilan',       Icon: BarChart3 },
            { key: 'calculateur', label: 'Calculateurs', Icon: Calculator },
            ...
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-colors inline-flex items-center justify-center gap-1.5 ${
                tab === t.key ? 'bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700'
              }`}>
